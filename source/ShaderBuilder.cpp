...
const char *Uniforms::addUniform(const char *name, GLSLType type, UniformValueFunction value) {
  const auto uniqueName = std::string(name) + (id != undefinedId ? "_" + std::to_string(id) : "");
...
void ShaderBuilder::appendGlobal(const std::string &name, GLSLType type, const std::string &value) {
